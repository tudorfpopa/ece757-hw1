...
int main( int argc, char** argv) {
  ...
  for(int c=mincutoff; c<maxcutoff; c+=50){
    ...
    // parallelize
    #pragma omp parallel for reduction(+:total_inplay_throws[:7], total_inplay_points[:7], total_inplay_duds[:7]) reduction(+:cutoffturns)
    for(int i=0;i<games;++i) {
      long long inplay_duds[7];
      long long inplay_throws[7];
      long long inplay_points[7];
      unsigned int seed = omp_get_thread_num();
      int turns = play_game(c, &seed, inplay_throws, inplay_points, inplay_duds);
      cutoffturns += turns;
#if VERBOSE

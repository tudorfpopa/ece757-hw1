...
\section{Running an OpenMP parallelized implementation of the game}
We repeated the same procedure, but using OpenMP to parallelize our program. The results are detailed on the graph below.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{../outputs/omp_plot.pdf}
    \caption{Speedup per number of threads (2-12)}
    \label{fig:omp}
\end{figure}
We can observe similar results to the pthreads implementation, where linear scaling occurs up to 5 threads.
\end{document}
